...
setInterval(() => {
const currentTime = new Date(); // Объект даты, возвращает текущее время
const day = namesOfDay[currentTime.getDay()]; // Метод возвращает день недели
const month = namesOfMonth[currentTime.getMonth()]; // Метод возвращает месяц
const year = currentTime.getFullYear(); // Год
const date = currentTime.getDate().toString().padStart(2, "0"); // Число месяца
...
refs.clock.textContent = `Current time: ${hours}: ${minutes}: ${seconds}`
...
//360/60 = 6 deg for seconds
//360/60 = 6 deg for minutes
//360/12 = 30 def for hours
...
// const idInt = setInterval(foo, 500, 'arg', 'arg2', 3);

// const idTim = setTimeout(foo, 300, 'Arr', user, true);
